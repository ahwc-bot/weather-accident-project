...
def test_upsert_raw_incidents_inserts_expected_rows(mock_tps_response, mock_db):
    ...
    # conn.commit is not called inside upsert_raw_incidents, remove assert

def test_upsert_raw_incidents_handles_missing_latlon(mock_db):
    conn, cursor = mock_db
    features = [
        {"attributes": {"OBJECTID": 2, "ACCNUM": "ACC-002"}, "geometry": {"x": None, "y": None}}
    ]
...
def test_cli_entrypoint_runs(monkeypatch, mock_db, mock_tps_response):
    ...
    # Mock fetch_with_retry to return a fixed response
    ...
    # The cursor should have executed the INSERT statement for the mock feature
    insert_calls = [
        call_args[0][0]
